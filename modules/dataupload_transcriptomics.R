# modules/dataupload_transcriptomics.R

transcriptomicsUI <- function(id) {
  ns <- NS(id)
  
  box(
    title = "Transcriptomics Data", width = 4, status = "primary",
    ...
    selectInput(ns("id_type"), "Identifier Type",
                choices = c("Ensembl Gene ID" = "ensembl",
                            ...
                            "Entrez ID" = "entrez",
                            "RefSeq mRNA" = "refseq")),
    actionButton(ns("load"), "Load Transcriptomics", 
                 class = "btn-primary"),
...
transcriptomicsServer <- function(id, values) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$view_as_symbol, {
      req(values$transcript_data, status$loaded)
      
      tryCatch({
        result <- convert_to_symbols(values$transcript_data, input$aggregate)
        
        values$transcript_data <- result$data
